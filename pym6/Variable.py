...
class GridVariable():
    ...
    def __init__(self, var, domain, loc, *fhl, **kwargs):
        for fh in fhl:
            try:
                ...
                for dim in self._dims:
                    try:
                        self.index_array.append(
                            [0, fh.dimensions[dim].dimtotlen])
                        setattr(self, 'total_' + dim[0] + 'len',
                                fh.dimensions[dim].dimtotlen)
                    except AttributeError:
                        self.index_array.append([0, fh.dimensions[dim].size])
                        setattr(self, 'total_' + dim[0] + 'len',
                                fh.dimensions[dim].size)
                self.index_array = np.array(self.index_array, dtype=np.int16)
            except KeyError:
                pass
            else:
                ...
                if np.any(np.isnan(self._plot_slice[:, :2].astype(float))):
                    indices = np.where(
                        np.isnan(self._plot_slice[:, :2].astype(float)))
                    self._plot_slice[:, :2][indices] = self.index_array[indices]
    ...
    def plot_slice(self, operation_string):
        ...
        limits = ['s', 'e']
        operations = {'m': -1, 'p': +1}
        self._plot_slice[axes.index(axis)][limits.index(limit)] += operations[
            operation]
        ...
        for axis in axes:
            for limit in limits:
                for op in operations:
                    ...
                    setattr(self, string,
                            partial(self.plot_slice_modifier, string))
    ...
    def extend_halos(self, array, axis, boundary_index, **kwargs):
        ...
        if method == 'vorticity':
            ...
            array_extendor = slip_multiplyer * array.take(
                [boundary_index], axis=axis)
        elif method == 'mirror':
            array_extendor = array.take([boundary_index], axis=axis)
        elif method == 'zeros':
            array_extendor = np.zeros(
                array.take(
                    [boundary_index], axis=axis).shape)
    ...
    def read_array(self, extend_kwargs={}, **kwargs):
        if np.any(self._plot_slice[:, 0] < 0):
            ...
            indices = np.where(self._plot_slice[:, 0] < 0)
            plot_slice_temp[:, 0][indices] = self.index_array[:, 0][indices]
        ...
        if self._divisor:
            ...
            if tmean:
                dt = self.average_DT
                divisor = np.apply_over_axes(np.sum, divisor * dt,
                                             0) / np.sum(dt)
        ...
        if self._plot_slice[1, 0] < 0:
            out_array = self.extend_halos(
                out_array, axis=1, boundary_index=0, **extend_kwargs)
        if self._plot_slice[1, 1] > self.total_zlen:
            out_array = self.extend_halos(
                out_array, axis=1, boundary_index=-1, **extend_kwargs)
        if self._plot_slice[2, 0] < 0:
            out_array = self.extend_halos(
                out_array, axis=2, boundary_index=0, **extend_kwargs)
        if self._plot_slice[2, 1] > self.total_ylen:
            out_array = self.extend_halos(
                out_array, axis=2, boundary_index=-1, **extend_kwargs)
        if self._plot_slice[3, 0] < 0:
            out_array = self.extend_halos(
                out_array, axis=3, boundary_index=0, **extend_kwargs)
        if self._plot_slice[3, 1] > self.total_xlen:
            out_array = self.extend_halos(
                out_array, axis=3, boundary_index=-1, **extend_kwargs)
    ...
    def _slice_array_to_slice(slice_array):
        ...
        Slice = np.s_[slice_array[0, 0]:slice_array[0, 1]:slice_array[
            0, 2], slice_array[1, 0]:slice_array[1, 1]:slice_array[
                1, 2], slice_array[2, 0]:slice_array[2, 1]:slice_array[2, 2],
    ...
    def o1diff(self, axis):
        possible_hlocs = dict(
            ...
            h=['h', 'h', 'v', 'u'],
            q=['q', 'q', 'u', 'v'])
        possible_vlocs = dict(l=['l', 'i', 'l', 'l'], i=['i', 'l', 'i', 'i'])
    ...
    def ddx(self, axis, **kwargs):
        div_by_area = kwargs.get('div_by_area', False)
        if div_by_area:
            possible_divisors = dict(
                u=[self.dom.dt, self.dom.db, self.dom.Aq, self.dom.Ah],
                v=[self.dom.dt, self.dom.db, self.dom.Ah, self.dom.Aq],
                h=[self.dom.dt, self.dom.db, self.dom.dyCv, self.dom.dxCu],
                q=[self.dom.dt, self.dom.db, self.dom.dyCu, self.dom.dxCv])
        else:
            possible_divisors = dict(
                u=[self.dom.dt, self.dom.db, self.dom.dyBu, self.dom.dxT],
                v=[self.dom.dt, self.dom.db, self.dom.dyT, self.dom.dxBu],
                h=[self.dom.dt, self.dom.db, self.dom.dyCv, self.dom.dxCu],
                q=[self.dom.dt, self.dom.db, self.dom.dyCu, self.dom.dxCv])
        possible_ns = dict(
            u=[0, 0, 0, 1], v=[0, 0, 1, 0], h=[0, 0, 0, 0], q=[0, 0, -1, -1])
        possible_ne = dict(
            u=[0, 0, -1, 0], v=[0, 0, 0, -1], h=[0, 0, -1, -1], q=[0, 0, 0, 0])
        if axis == 1:
            ...
            divisor = possible_divisors[self.values.loc[0]][axis][self._slice[
                2:]]
            extend_kwargs = {'method': 'mirror'}
            if self._plot_slice[2, 0] < 0:
                divisor = self.extend_halos(
                    divisor, axis=0, boundary_index=0, **extend_kwargs)
            if self._plot_slice[2, 1] > self.total_ylen:
                divisor = self.extend_halos(
                    divisor, axis=0, boundary_index=-1, **extend_kwargs)
            if self._plot_slice[3, 0] < 0:
                divisor = self.extend_halos(
                    divisor, axis=1, boundary_index=0, **extend_kwargs)
            if self._plot_slice[3, 1] > self.total_xlen:
                divisor = self.extend_halos(
                    divisor, axis=1, boundary_index=-1, **extend_kwargs)
    ...
    def move_to(self, new_loc):
        ...
        out_array = 0.5 * (np.take(
            ...
            axis=axis) + np.take(
                self.values, range(1, self.values.shape[axis]), axis=axis))
