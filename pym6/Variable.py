...
class GridVariable():
        ...
        for fh in fhl:
            try:
                ...
                for dim in self._dims:
                    try:
                        self.index_array.append([0,fh.dimensions[dim].dimtotlen])
                ...
                if np.any(np.isnan(self._plot_slice[:,:2].astype(float))):
                    indices = np.where(np.isnan(self._plot_slice[:,:2].astype(float)))
    ...
    def read_array(self,extend_kwargs={},**kwargs):
        ...
        if self._plot_slice[1,0] < 0:
            out_array = self.extend_halos(out_array,axis=1,
    ...
    def ddx(self,axis,**kwargs):
        ...
        if div_by_area:
            possible_divisors = dict(u = [self.dom.dt, self.dom.db,
                                          self.dom.Aq, self.dom.Ah],
                                    v = [self.dom.dt, self.dom.db,
                                         self.dom.Ah,  self.dom.Aq],
